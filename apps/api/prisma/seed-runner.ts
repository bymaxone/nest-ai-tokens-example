 ...
 * seeded wallet id, wallets by seeded tenant) before the plan is re-inserted
 * with its stable ids, so any number of runs converges to the same state.
 *
 * @layer service
 */
 ...
 *
 * Deletes children before parents (entries before wallets) to respect the
 * foreign keys, then inserts parents before children.
 *
