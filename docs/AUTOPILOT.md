...
- **Project root**: /Users/maximiliano/Documents/MyApps/bymax-one/nest-ai-tokens-example
...
<!-- Each row: when it applies, the exact check command (run from Project root),
     and what to do on failure. STOP is the default — autopilot never polls for
     external events the repo cannot influence. -->
...
> their working copy lives in a temporary worktree, NOT beside `../nest-ai-tokens`
> — the relative `file:` path will **not** resolve there, and the same is true
> in GitHub Actions CI (which checks out only this repo). Phase 01's Definition
> of Done ("subpath probe passes in CI") cannot hold as written under either
> condition. **Pick one before running:**
>
> 1. **Publish** `@bymax-one/nest-ai-tokens@0.1.0` to a registry (private
>    GitHub Packages) and switch the dep to `^0.1.0` — then worktrees and CI
...
| 01 Postgres, Prisma & Library Link | `inherit` | first contact with the consumed library — subpath resolution + `file:`-link gotchas are the failure mode; exact Prisma reference shapes |
...
| 03 Repositories, Ledger & Pricing | `inherit` | implements both library repository ports (14 methods) with SQL aggregation, race-safe upsert and window logic — contract fidelity + money-adjacent correctness |
...
| 05 Quota, Credits & Aggregations | `inherit` | security-sensitive: quota guard enforcement (drain→402), ledger-backed balance resolver, credit/refund money paths — a /security-review would flag all of these |
...
| 07 Web Skeleton & Design System | `sonnet` | UI shell on an established API + verbatim shared design system |
| 08 Dashboard Pages | `sonnet` | UI pages (feature work) on established endpoints + the design system |
...
<!-- Mechanically checkable project rules — each command must print NOTHING.
     Run these in every implementer's phase-wide gate. Paths are relative to the
     project root; apps appear from phase 01 onward. -->

```bash
# Web must import ONLY the ./shared subpath, never the server root '.'
grep -rn "from '@bymax-one/nest-ai-tokens'" apps/web 2>/dev/null

# No suppression comments anywhere (types or lint)
grep -rnE '@ts-ignore|@ts-nocheck|@ts-expect-error|eslint-disable' apps --include='*.ts' --include='*.tsx' 2>/dev/null

# process.env confined to the config/env layer (ai-tokens.config.ts + env schema)
grep -rn "process\.env" apps --include='*.ts' 2>/dev/null | grep -vE 'ai-tokens\.config\.ts|env\.(ts|schema\.ts)|\.spec\.ts|\.e2e-spec\.ts'

# No placeholder files (user-global rule + plan §9)
find . -name '.gitkeep' -o -name '.keep' 2>/dev/null | grep -v node_modules

# No real provider secret committed anywhere
grep -rnE 'sk-[A-Za-z0-9]{20,}' apps 2>/dev/null
...
- **No shortcuts**: no `@ts-ignore` / `eslint-disable` / `--no-verify` /
  `#[allow]` / skipped hooks / lowered coverage thresholds — in implementer work
  or in what the orchestrator accepts.
